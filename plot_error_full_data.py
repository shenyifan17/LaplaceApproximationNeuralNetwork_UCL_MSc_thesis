...
plt.grid(True)
plt.title('log_scale_eig_spectrum_full_data_fc')
plt.xlabel('dimension')
plt.ylabel('log_eigenvalue')
plt.savefig('eig_spectrum_full_data_fc.png')
plt.close()
...
# plt.ylabel('accuracy')
# plt.title('learning_curve_full_data')
# plt.savefig('learning_curve_full_data.png')
# plt.close()
...
def plot_error(mode='eig_large', which=5000):
  ...
  ax.legend()
  ax.grid(True)
  plt.title(str(which+1)+ '_full_eigval = ' + str(eig_value))
  plt.xlabel('distance_along_' + str(which+1) + '_eigenvector')
  plt.ylabel('error')
  plt.savefig('fc_' + str(which+1)+ '_full_comparison.png')
  plt.close()
