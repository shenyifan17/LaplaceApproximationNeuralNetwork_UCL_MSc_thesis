...
with tf.train.MonitoredSession() as sess:
  ...
  while mnist.train.epochs_completed < num_epochs:
    ...
    if i % log_period_updates == 0:
      ...
      train_acc = accuracy.eval(session = sess, feed_dict={x: compress_mnist(mnist.train.images), y_: mnist.train.labels})
...
plt.grid(True)
plt.title('log_scale_eig_spectrum_full_data_cov')
plt.xlabel('dimension')
plt.ylabel('log_eigenvalue')
plt.savefig('eig_spectrum_full_data_cov.png')
plt.close()

# plt.plot(train_accuracy, c='blue', label='train_accuracy')
# plt.plot(test_accuracy, c='red', label='test_accuracy')
# plt.grid(True)
# plt.title('cov_learning_curve_full_data')
# plt.xlabel('number_of_steps (in thousands)')
# plt.ylabel('accuracy')
# plt.title('learning_curve_full_data')
# plt.savefig('cov_learning_curve_full_data.png')
# plt.close()
...
def plot_error(mode='eig_large', which=5000):
  ...
  ax.legend()
  ax.grid(True)
  plt.title('cov_' + str(which+1)+ '_full_eigval = ' + str(eig_value))
  plt.xlabel('distance_along_' + str(which+1) + '_eigenvector')
  plt.ylabel('error')
  plt.savefig('cov_' + str(which+1)+ '_full_comparison.png')
  plt.close()
